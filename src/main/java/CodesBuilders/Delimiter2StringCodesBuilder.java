...
public class Delimiter2StringCodesBuilder {
    ...
    public static Integer getIntegerForCode(String code){
        return Delimiter2Amounts.getAmountForCurrentSize(code.length() - 1) + Delimiter2Codes.getPositionForCurrentSize(code);
    }
    ...
    public static String getCodeForInteger(Integer number){
        ...
        for (int size = 3; ; size++){
            if (number < Delimiter2Amounts.getAmountForCurrentSize(size)){
                return Delimiter2Codes.getCodesForCurrentSize(size).getCodes().get(number - Delimiter2Amounts.getAmountForCurrentSize(size - 1));
            }
        }
    }
